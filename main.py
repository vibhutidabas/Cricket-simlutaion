...
t1 = []
name1=["Faf du Plessis","Virat Kohli","Glenn Maxwell","Mahipal Lomror","Dinesh Karthik","Wanindu Hasaranga","Shahbaz Ahmed","Harshal Patel","Karn Sharma","Mohammed Siraj","Josh Hazlewood"]
for i in range(10):
    t1.append(Player(name1[i], round(random.random(),1), round(random.random(),1),round(random.random(),1), round(random.random(),1), round(random.random(),1)))

t2 = []
name2=["MS Dhoni","Devon Conway","Ruturaj Gaikwad","Ben Stokes","Ambati Rayudu","Shivam Dube","Moeen Ali","Ravindra Jadeja","Deepak Chahar","Mitchell Santner","Rajvardhan Hangargekar"]
for i in range(10):
    t2.append(Player(name2[i], round(random.random(),1), round(random.random(),1),round(random.random(),1), round(random.random(),1), round(random.random(),1)))
